...
class PartnerController extends Controller
{
    ...
    public function purchaserApply(Request $request)
    {
        $data = $request->validate([
            ...
            'company_name' => '상호명', 'owner_name' => '대표자명', 'business_no' => '사업자번호',
            'phone' => '연락처', 'email' => '이메일', 'password' => '비밀번호',
        ]);

        $slug = Str::slug($data['company_name']);
        if ($slug === '' || Purchaser::where('slug', $slug)->exists()) {
            $slug = 'buyer-agent-'.Str::lower(Str::random(6));
        }

        $purchaser = Purchaser::create([
            'name' => $data['company_name'], 'slug' => $slug, 'status' => 'pending',
            'cashback_rate' => 5, 'owner_name' => $data['owner_name'],
            'business_no' => $data['business_no'], 'phone' => $data['phone'], 'email' => $data['email'],
        ]);

        User::create([
            'name' => $data['company_name'], 'email' => $data['email'],
            'role' => 'purchaser', 'purchaser_id' => $purchaser->id, 'password' => Hash::make($data['password']),
        ]);

        return redirect()->route('manage.login')
            ->with('status', '구매 대행자 신청이 완료되었습니다. 본사 승인 후 로그인하실 수 있습니다.');
    }
}
