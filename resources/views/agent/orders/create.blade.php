...
@section('title', '주문 등록')
@section('page', '주문 등록')
@section('crumb', '거래처 영업 주문 등록')

@php $rate = (float) auth()->user()->agent->commission_rate; @endphp

@section('content')
@if($clients->isEmpty())
    <div class="panel"><div class="panel-b empty">
        먼저 거래처를 등록해야 주문을 등록할 수 있습니다.
        <div style="margin-top:14px"><a href="{{ route('agent.clients.create') }}" class="btn btn-accent">거래처 등록하러 가기</a></div>
    </div></div>
@else
                ...
                <div class="panel-b">
                    ...
                    <div class="form-row">
                        ...
                        <select class="select" name="status">
                            <option value="pending">주문 접수 (커미션 적립 대기)</option>
                            <option value="paid">결제 완료 (커미션 적립)</option>
                        </select>
                        <div class="hint">결제완료 상태의 주문만 커미션이 적립됩니다.</div>
                    </div>
                </div>
            </div>
            <div class="panel">
                <div class="panel-b">
                    <div class="summary-row" style="display:flex;justify-content:space-between;padding:8px 0"><span class="muted">판매금액</span><b id="sumTotal">0원</b></div>
                    <div class="summary-row" style="display:flex;justify-content:space-between;padding:8px 0;border-top:1px solid var(--line);margin-top:6px">
                        <span class="muted">예상 커미션 ({{ rtrim(rtrim(number_format($rate,2),'0'),'.') }}%)</span>
...
(function(){
    var RATE = {{ $rate }};
    var idx = 0;
    var searchUrl = "{{ route('agent.orders.search') }}";
