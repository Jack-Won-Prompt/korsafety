{{--
    ...
    $product : 대상 상품
    $ajax    : true면 저장 후 페이지 이동 없이 결과 URL을 돌려받는다 (모달용)
--}}
@php $ajax = $ajax ?? false; @endphp
<div class="ed-grid">
    <div>
        <div class="ed-stage">
    ...
    <div>
        <form action="{{ route('manage.products.image.save', $product) }}" method="post" id="edForm"
              data-ajax="{{ $ajax ? '1' : '' }}" data-src="{{ asset($product->main_image) }}">
            @csrf
...
(function(){
    ...
    var base = new Image();
    var rot = 0, bri = 100, con = 100;      // rot = 미세(-45~45)
    ...
    function resetAll(src){
        base = new Image(); base.onload=render; base.src = src || SRC;
    ...
    document.getElementById('resetBtn').onclick=function(){ resetAll(); };
    ...
    form.addEventListener('submit', function(e){
        ...
        if(!AJAX) return;   // 단독 페이지는 그대로 폼 전송
        ...
        fetch(form.action, {
          ...
          .then(function(res){
              ...
              if(!res.ok || !res.d.url){ alert(res.d.message || '이미지 저장에 실패했습니다.'); return; }
              // 폼의 대표 이미지 미리보기를 새 이미지로 교체하고 모달을 닫는다
              document.querySelectorAll('[data-main-preview]').forEach(function(img){ img.src = res.d.url; });
              SRC = res.d.url;
              resetAll(res.d.url);
              if(window.closeImageModal) window.closeImageModal();
              if(window.manageToast) window.manageToast('대표 이미지가 저장되었습니다.');
          })
          .catch(function(){
              btn.disabled = false; btn.textContent = '저장';
